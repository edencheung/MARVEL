...
full_conv_log = open(f'results/logs/{IP}_{MODEL}_{AGENT}_full_conv_log.txt','w')
def log_full_conv_message(message: str):
    """Log a message to the full conversation log."""
    full_conv_log.write(message+'\n')
    full_conv_log.flush()

main_conv_log = open(f'results/logs/{IP}_{MODEL}_{AGENT}_main_conv_log.txt','w')
def log_main_conv_message(message: str):
    """Log a message to the main conversation log."""
    main_conv_log.write(message+'\n')
    main_conv_log.flush()

actions_log = open(f'results/logs/{IP}_{MODEL}_{AGENT}_actions_log.txt','w')
